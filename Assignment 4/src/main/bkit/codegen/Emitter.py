...
class Emitter():
    ...
    *   @param in the type descriptor of the method.
    ...
    def emitINVOKESTATIC(self, lexeme, in_, frame):
        ...
        typ = in_
        list(map(lambda x: frame.pop(), typ.intype))
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        if type(in_) is not cgen.VoidType:
            frame.pop()
        if type(in_) is cgen.VoidType:
            return self.jvm.emitRETURN()
        elif type(in_) is cgen.FloatType:
            return self.jvm.emitFRETURN()
        elif type(in_) in [cgen.IntType, cgen.BoolType]:
    ...
    *   @param in the code to be printed out-
    ...
    def emitNEWARRAY(self, typ, frame):
        ...
        if type(typ) is cgen.IntType:
            ...
            tp = 'java/lang/String'
        result.append(self.jvm.emitNEWARRAY(tp))
        return ''.join(result)
